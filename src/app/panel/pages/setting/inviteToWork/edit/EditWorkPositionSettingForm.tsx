...
const EditWorkPositionSettingForm: React.FC<{
  ...
  const getAllWorkPositionAgain = useCallback(() => {
    return getAllWorkPosition();
  }, [updateCard]);

  const handleUpdateCard = () => setUpdateCard((prev) => !prev);

  const { data, loading } = useFetch(getAllWorkPositionAgain);
  ...
  const handleDeleteItem = async (id: number) => {
    ...
    try {
      const response = await removeWorkPosition(id);
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await updateWorkPosition(newObj);
      toast.dismiss(loading);
            ...
            <div className="flex items-center justify-between px-2 py-4 border rounded-md bg-light-border border-middle-border">
              ...
              <div className="flex gap-3">
                ...
                <Popconfirm
                  okText="بله"
                  cancelText="خیر"
                  title="آیا از حذف این سمت شغلی اطمینان دارید؟"
...
export default EditWorkPositionSettingForm;
