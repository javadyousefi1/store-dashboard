...
const EditFieldOfStudySettingForm: React.FC<{
  isWorkPositionInputVisible: boolean;
  setIsWorkPositionInputVisible: (arg0: boolean) => void;
}> = ({ isWorkPositionInputVisible, setIsWorkPositionInputVisible }) => {
  const [id, setId] = useState(0);
  const [updateCard, setUpdateCard] = useState(false);

  const getAllFieldOfStudyAgain = useCallback(() => {
    return getAllFieldOfStudy();
  }, [updateCard]);

  const handleUpdateCard = () => setUpdateCard((prev) => !prev);

  const { data, loading } = useFetch(getAllFieldOfStudyAgain);
  const handleConvert = (id: number) => {
    setId(id);
    setIsWorkPositionInputVisible(true);
  };

  const handleDeleteItem = async (id: number) => {
    const loading = toast.loading("در حال ثبت");
    try {
      const response = await removeFieldOfStudy(id);
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await updateFieldOfStudy(newObj);
      toast.dismiss(loading);
      toast.success("ویرایش با موفقیت ثبت شد");
      setIsWorkPositionInputVisible(false);
    } catch (error) {
      toast.dismiss(loading);
      toast.error("ویرایش سمت شغلی با خطا مواجه شد");
    }
    handleUpdateCard();
  };
  ...
  return data ? (
    <div className="flex flex-col gap-2">
      {data?.data?.items?.map((item, i) => (
        <>
          {isWorkPositionInputVisible && id === item.id ? (
            <Form
              initialValues={{
                title: item?.title,
            ...
            <div className="flex items-center justify-between px-2 py-4 border rounded-md bg-light-border border-middle-border">
              ...
              <div className="flex gap-3">
                ...
                <Popconfirm
                  okText="بله"
                  cancelText="خیر"
                  title="آیا از حذف این رشته تحصیلی اطمینان دارید؟"
...
export default EditFieldOfStudySettingForm;
