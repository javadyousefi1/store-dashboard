...
import { deleteAccident } from "@/api/accident";
// component
...
// form
import EditAccident from "./forms/EditAccident";
...
import AddNewsFromAccident from "./forms/AddNewsFromAccident";
...
const TableAction: React.FC<{
  ...
  const handleDeleteAccident = () => {
    const loadingToast = toast.loading("در حال حذف حادثه ");

    deleteAccident({ id: rowData.id })
      .then(() => {
        toast.dismiss(loadingToast);
        toast.success("حادثه با موفقیت حذف شد");
        handleRefreshTable();
      })
      .catch(() => {
        toast.dismiss(loadingToast);
        toast.error("حذف حادثه با خطا مواجه شد");
      });
  };
  ...
  const [isSendToNewsModalOpen, setIsSendToNewsModalOpen] = useState(false);
  ...
  const hideModal = () => {
    ...
    setIsSendToNewsModalOpen(false);
  };
  ...
  const handleShowNewsModal = () => {
    setIsSendToNewsModalOpen(true);
  };

  // useEffect(() => {
  //   if (!loading) {
  //     form.setFieldsValue({
  //       banner: data?.data?.files.map((item) => ({
  //         uid: `-${item.id}`,
  //         name: item.path,
  //         status: "done",
  //         url: item.path,
  ...
  return (
    <div className="flex items-center justify-center gap-x-6">
      {/* delete */}
      <Tooltip title="حذف حادثه">
        <Popconfirm
          title="حذف حادثه"
          description="آیا از حذف حادثه اطمینان دارید"
          okText="بله"
          cancelText="خیر"
          onConfirm={handleDeleteAccident}
        >
          <button>
            <svg
              width="20"
      ...
      <Tooltip title="جزئیات حادثه">
        <button type="button" onClick={showModal}>
          ...
          >
            <path
              fill="#444444"
              d="M9 12a1 1 0 1 1-2 0a1 1 0 0 1 2 0m4 0a1 1 0 1 1-2 0a1 1 0 0 1 2 0m3 1a1 1 0 1 0 0-2a1 1 0 0 0 0 2"
            />
            <path
              fill="#444444"
              fill-rule="evenodd"
      ...
      >
        <AddNewsFromAccident
          handleRefreshTable={handleRefreshTable}
          rowData={rowData}
          accidentId={rowData?.id}
          // accidentData = {accidentData}
          hideModal={hideModal}
        />
        {/* <NewsForm
          fetchFunction={createNewsFromAccident}
          accidentId={rowData?.id}
          hideModal={hideModal}
          handleRefreshTable={handleRefreshTable}
        /> */}
